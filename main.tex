...
\abstract
% context
{Star clusters survive instantaneous gas expulsion more easily when star
formation proceeds with a centrally peaked star formation efficiency
(SFE), because the stars are then born more concentrated than the gas
they must unbind from. The observationally motivated, spatially
truncated King family has not been mapped in this respect, even though
it is the family adopted by upcoming $N$-body campaigns.}
% aims
...
(up to quadratures) the residual-gas density profile for King models
with $W_0=0.5$--$20$, compute the post-expulsion virial ratio --
validated against the effective SFEs inferred at the published Plummer
and Dehnen $N$-body thresholds -- and introduce a scale-free criterion
that removes gas whose local
free-fall time exceeds the star formation duration. Independently, we
compute the bound mass fraction from a frozen stellar distribution
function obtained by an analytic Eddington inversion in the total
star--gas potential, and cross-check it against Monte Carlo
realizations drawn from the same distribution function.}
% results
{The virial criterion yields an SFE threshold that is a non-monotonic
function of $W_0$, falling to a minimum of $\simeq0.11$ near
$W_0\simeq8.8$ for the reference choice $\esfe_{\rm surv}=1/3$, and rising
again toward high concentration. The dependence reduces exactly
to a single dimensionless parameter, the normalized star--gas cross
energy per unit gas mass, whose minimum is inherited from the maximum of
the King structural ratio $\rtid/\rhalf$, where gas--star segregation is
greatest. The result is robust to the free-fall-time truncation. The
frozen-DF analysis reproduces the same non-monotonic trend by an
independent criterion, and predicts that highly concentrated models keep
a self-bound central seed down to global SFEs of a few per cent. The
survival value of the effective SFE is shown to be non-universal across
density profiles.}
% conclusions
...
available at \url{https://github.com/BS-astronomer/kingmodels}.
